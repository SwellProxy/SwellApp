...
面向零技术基础用户的 iOS 代理 App。**UIKit** + sing-box(Libbox)。

---

## 0. 🔴 技术栈决策:UIKit 为主,不要改成 SwiftUI

**已定,有实测依据,不要再提议换 SwiftUI。**

| 项 | 决策 |
|---|---|
| UI 框架 | **UIKit**(工程已是 UIKit + Storyboard 模板) |
| SwiftUI | **仅按需嵌入**,通过 `UIHostingController`。典型场景:图表(如 swift-charts)、纯静态展示卡片 |

**理由(维护者有过同类 App 的实测经验):** SwiftUI 的性能追不上 UIKit,**尤其是长列表滑动**。

**对本项目为什么特别成立:** 屏 6 节点列表是最吃性能的一屏 —— 48+ 条双行 cell,还要在测速时**实时回填延迟**。这正是 SwiftUI `List` 的短板、UIKit cell 复用的强项。

### 因此的实现建议

- **节点列表(屏 6):** `UICollectionView` + `UICollectionViewCompositionalLayout` + `UICollectionViewDiffableDataSource`
  - 订阅分组头用 supplementary view(含那条 2px 进度条)
  - **测速回填延迟时用 `reconfigureItems`(iOS 15+)做局部刷新,不要 `reloadData` / 不要重建 snapshot** —— 全组测速时会高频更新,重建会掉帧
- **分流页 / 设置页:** 列表短,`UICollectionView` list configuration 即可
- **连接页(屏 4/5):** 非列表,手写 UIKit;注意两态卡片**位置固定、内容生长**,不要重排
- **Liquid Glass:** 设计稿是 iOS 26 Liquid Glass 观感。UIKit 侧的对应 API 请先查证当前 SDK 再实现,**不要凭印象写**
...
SwellApp/
├── CLAUDE.md              ← 本文件
├── .gitignore             ← 已排除 xcuserdata / 构建产物 / 签名文件
├── design/                ← UI 规格(设计已封板)
...
> ⚠️ **HTML → UIKit 是照着重写,不是转换。** 从 HTML/CSS 里读精确的色值、字号、间距、圆角,用 UIKit 重新实现。不要试图移植 DOM 结构,也不要因为设计是 HTML 就联想到 SwiftUI(见 §0)。
...
- **§0 的 UIKit 决策是定了的**,不要提议改 SwiftUI。需要 SwiftUI 时用 `UIHostingController` 局部嵌入
...
- 涉及当前 SDK 的新 API(如 Liquid Glass),**先查证再写**,不要凭印象
